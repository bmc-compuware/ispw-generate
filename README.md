...
| `generate_automatically` | Optional | A string of JSON that contains the parameters for the generate. If using an code-pipeline-sync step before the generate, this JSON string can be retrieved from the outputs of that step. If `generate_automatically` is not being used, then the `assignment_id` and `level` must be specified. |
...
| `task_id` | Optional | The comma-separated string of task IDs for the tasks that need to be generated. Do not use if `generate_automatically` has already been specified. Otherwise, if not specified the action will generate all the tasks at specified level in an assignment container. |
